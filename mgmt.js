...
window.renderForecastView = () => {
    ...
    if (sales.length < 30) {
        return '<div style="max-width:900px;margin:auto;"><div class="card" style="text-align:center;padding:40px;">' +
            ...
            '<p style="color:var(--text-muted);font-size:13px;margin-top:8px;">Need at least 30 days of takings dat// --- 1. TAKINGS & KPI DASHBOARD ---
window._salesTab = window._salesTab || 'week';
window._salesMonth = window._salesMonth || null; // 'YYYY-MM' format
...
window.deleteRoster = (i) => { if(confirm("Delete roster?")) { window.shiftRosters.splice(i,1); window.saveToDisk(); window.showView('rosters'); } };a to generate forecasts.</p>' +
            '</div></div>';
    }
