...
window.editStaffForm = (idx) => {
    ...
    const html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">' +
        ...
        '<div><label style="font-size:11px;color:var(--text-muted);">Role</label><select id="sd-role" class="input-box" style="margin:0;">' + (function(){var r=Object.keys((window.staffHubConfig&&window.staffHubConfig.roles)||{});['FOH','BOH','Bar','Manager','Director','Kitchen Hand'].forEach(function(d){if(!r.includes(d))r.push(d);});return r;})().map(r => '<option ' + (s.role===r?'selected':'') + '>' + r + '</option>').join('') + '</select></div>' +
